...
import os  # for deleting the file created by previous running of the program
...
def get_initial():
    print("Please enter a number from 0 to 8. No number should be repeated or be out of the said range.")
    initial_state = np.zeros(9)
    for i in range(9):
        states = int(input("Enter the " + str(i + 1) + " number: "))
        if states < 0 or states > 8:
            print("Please only enter states which are 0 to 8, run the code again.")
            exit(0)
        else:
...
def write_path(path_formed):  # The function is to write the final path in the text file.
...
def write_node_explored(explored):  # The function is to write all the nodes explored by this program.
...
def write_node_info(visited):  # The function is to write all the info about the nodes explored by this program.
...
def path(node):  # The function is to find the path from the goal node to the starting node.
    p = []  # Empty list
    p.append(node)
...
def exploring_nodes(node):
    ...
    final_nodes = []
    visited = []
    final_nodes.append(node_q[0].data.tolist())# The only writing data of nodes in seen
    node_counter = 0  # The function is to define a unique ID to all the nodes formed.
    ...
    while node_q:
        ...
        for move in actions:
            ...
            if temp_data is not None:
                node_counter += 1
                child_node = Node(node_counter, np.array(temp_data), current_root, move, 0)  # This creates a child node.
    ...
    return None, None, None  # If the goal node is not reached, then return statement.
